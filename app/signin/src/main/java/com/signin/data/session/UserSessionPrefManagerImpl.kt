...
internal class UserSessionPrefManagerImpl @Inject constructor(
    ...
    override fun loginUser(loggedInUser: LoggedInUser): Boolean {
        return sharedPreferences.edit().putString(PREF_USER_SESSION, gson.toJson(loggedInUser))
            .commit()
    }

    override fun getLoginUserId(): String {
        val session = sharedPreferences.getString(PREF_USER_SESSION, null)
        return gson.fromJson(session, LoggedInUser::class.java).userId
    }
}